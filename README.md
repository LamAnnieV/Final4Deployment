...
The fairly new Microsoft application framework adds flexibility for developer use and lower latency for user experience.  Our interest in an ASP.Net core application circled around what it could do for the goals of a business since they can be deployed on multiples OS systems compared to traditional ASP.Net framework apps that can only be used with Windows. This adds flexibility in building the infrastructure for an application and gives a business the abilitiy to span their networking over a longer range. AS{.Net core applications combine MVC(Model-View-Controller) and Web API components that enable the functionality for an application with a microservice architecture. The controller in this applicaiton is the REST API that gets installed as a dependency for asp.net core applications and manipulates the model or the Microsoft Azure SQL database by storing and retrieving data from it through the API requests, which updates the view of the client through the webpage that is served up by the kestrel web server upon client requests. The eShoponweb application that we chose was based in a serverless Microsoft Azure SQL database in the cloud the sends and recieves HTTP requests to our application. Our application almost comes containerized in that its programmimg code comes with dependencies that include the kestrel web server, a .NET core runtime engine that is the core processor of the applicaiton, a NuGet package manager that takes care of downloading and installing the necessary dependencies giving the developer more time to focus on the infrastrcture, and middleware that executes code and handles error through a route handler. This handles or controls the HTTP requests and responses between the API and the database. Once fetched, the response from the database goes back to the API that routes them to the kestrel web server upon client request connected to its respective microservice. 
